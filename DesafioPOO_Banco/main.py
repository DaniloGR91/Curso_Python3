...
def menu():			
	while True:
		...
		if menu.opcao == 3:
			...
			validar_cliente = banco.validar_cliente(str(numero_conta_cliente), agencia_cliente)

			global conta_cliente
			if validar_cliente[0] and not validar_cliente[1]:
				cliente = banco.conectar_cliente(str(numero_conta_cliente), 'corrente')
				...
				cliente = banco.conectar_cliente(str(numero_conta_cliente), 'poupanca')
				...
				while True:
					conta_escolhida = input('Você deseja acessar qual conta? [1 - Conta Corrente; 2 - Conta Poupança] ').strip()
