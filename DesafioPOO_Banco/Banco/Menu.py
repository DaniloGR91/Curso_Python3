			...
			if self.agencia_atual == 1111:
				...
				while True:
					...
					if self.validar_nova_agencia(agencia_nome, agencia_numero):
						super().cadastrar_agencia(agencia_nome=agencia_nome, agencia_numero=int(agencia_numero))
						break
			else:
				print('Apenas a agência 1111 tem autorização para cadastrar uma nova agência')
